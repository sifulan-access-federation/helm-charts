...
{{- define "osf.external.environment" -}}
...
- name: {{ $key }}
  valueFrom:
    secretKeyRef:
      ...
      key: {{ $key }}
{{- end }}
{{- include "osf.external.environment" . -}}
{{- end -}}
...
- name: secret
  secret:
    secretName: {{ template "osf.fullname" . }}-migration
{{- end -}}
...
{{- define "osf.admin.apache.environment" -}}
{{- $fullname := include "osf.admin.fullname" . -}}
...
- name: {{ $key }}
  valueFrom:
    configMapKeyRef:
      ...
      key: admin-{{ $key }}
{{- end }}
{{- range $key, $value := .Values.admin.apache.secretEnvs }}
- name: {{ $key }}
  valueFrom:
    secretKeyRef:
      name: {{ $fullname }}
...
{{- define "osf.admin.apache.volumeMounts" }}
{{- range $key := keys (merge .Values.admin.apache.configFiles (include "osf.admin.inlineconfigs" . | fromYaml) (include "osf.admin.fileconfigs" . | fromYaml)) }}
- mountPath: /etc/{{ $key }}
  name: config
  subPath: admin-{{ $key | replace "/" "-" }}
  readOnly: true
{{- end }}
{{- range $key := keys .Values.admin.apache.secretFiles }}
- mountPath: /etc/{{ $key }}
  name: secret
  subPath: admin-{{ $key | replace "/" "-" }}
  readOnly: true
